...
- Tổng tiền cọc đang khóa trong Solana Escrow PDA: ${totalLockedUsdc} SOL
- Tỷ giá quy đổi: 1 SOL = ${exchangeRate} VND
...
Hãy phân tích và trả về một đối tượng JSON hợp lệ duy nhất có định dạng sau (không chứa markdown \`\`\`json):
{
  ...
  "tien_giai_ngan_usdc": số tiền chuyển cho Nông dân (number) (Tính bằng SOL),
  "tien_hoan_usdc": số tiền hoàn trả cho Thương lái (number) (Tính bằng SOL),
  "ly_do": "Giải trình chi tiết bằng tiếng Việt: Nêu rõ lượng giao nhận thực tế (đạt bao nhiêu %), lỗi chất lượng phát hiện, đối chiếu với điều khoản nào và phạt bao nhiêu %, và công thức tính ra số tiền cuối cùng cho mỗi bên theo đơn vị SOL."
}
`;
