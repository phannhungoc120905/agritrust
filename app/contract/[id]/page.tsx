...
function ContractPageContent() {
    ...
    const loadData = async () => {
        try {
            ...
            try {
                const { getTranscriptsByContractId } =
                    await import("../../../lib/supabase/queries/transcripts");
    ...
    useEffect(() => {
        ...
        const contractChannel = supabase
            ...
            .on(
                ...
                (payload) => {
                    console.log(
                        "Realtime: phát hiện thay đổi báo cáo tranh chấp:",
                        payload,
                    );
                    loadData();
                },
            ...
            .on(
                ...
                (payload) => {
                    console.log(
                        "Realtime: phát hiện thay đổi hợp đồng:",
                        payload,
                    );
                    loadData();
                },
    ...
    const handleConfirmReceipt = async () => {
        try {
            ...
            if (result.success) {
                ...
                setSuccessMsg(
                    "✓ Giải ngân 100% thành công! Tiền từ Escrow PDA đã được chuyển vào ví Nông dân.",
                );
        ...
        return (
            <div className="flex-1 flex items-center justify-center bg-white text-neutral-400 min-h-screen gap-2">
                ...
                <span className="text-sm font-semibold">
                    Đang truy xuất thông tin tài khoản ký quỹ...
                </span>
            </div>
        );
    }
        ...
        <div className="flex-1 flex flex-col min-h-screen bg-[#FBFBFA]">
            ...
            <header className="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm">
                <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
                    <Link
                        href="/"
                        className="flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-lg border border-slate-250 bg-slate-50 hover:bg-slate-100 active:bg-slate-200 text-slate-700 transition-all duration-200 shadow-sm hover:shadow-md cursor-pointer"
                    >
                        <ArrowLeft size={14} className="text-slate-650" />{" "}
                        {isEnglish ? "Back to Home" : "Trở lại Trang chủ"}
                    </Link>
                    ...
                    <div className="flex items-center gap-2 text-xs font-bold text-neutral-500">
                        ...
                        <div className="flex items-center gap-1.5 relative">
                            <span
                                className="font-mono text-neutral-900 bg-neutral-100 px-2 py-1 rounded select-all"
                                title={contract.id}
                            >
                            ...
                            <div>
                                <h3 className="font-extrabold text-neutral-900 text-[15px] flex items-center gap-2">
                                    ...
                                    {isEnglish
                                        ...
                                        : "Hồ sơ Hợp đồng Ký quỹ"}
                                </h3>
                                <p className="text-[11px] text-neutral-450 mt-0.5">
                                    {isEnglish
                                        ? "Simulated contract document with blockchain signatures"
                                        : "Văn bản mô phỏng hợp đồng có chữ ký số Blockchain"}
                                </p>
                            </div>
                            <div className="flex flex-wrap items-center gap-4">
                                <div className="flex border-b border-slate-200">
                                    ...
                                    >
                                        ...
                                        {viewMode === "paper" && (
                                            <span className="absolute bottom-0 left-0 right-0 h-[2.5px] bg-[#2e7d32] rounded-full" />
                                        )}
                                    </button>
                                    <button
                                ...
                                <span
                                    className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider border shadow-sm shrink-0 whitespace-nowrap ${
                                ...
                                >
                                    {contract.trang_thai === "da_khoa_tien"
                                        ? isEnglish
                                            ...
                                            : "Đã ký quỹ (Locked)"
                                        : contract.trang_thai === "da_xac_nhan"
                                          ? "✅ Đã giải ngân 100%"
                                          : contract.trang_thai ===
                                              "da_giai_quyet"
                                            ? "⚖️ Đã phân xử"
                        ...
                        {viewMode === "paper" ? (
                            <div className="print:m-0 print:max-h-none print:overflow-visible max-h-[75vh] overflow-y-auto">
                                <DraftContractTable
                                    ...
                                    currentRole={
                                        isNongDan ? "nong_dan" : "thuong_lai"
                                    }
                                />
                            </div>
                        ) : (
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 bg-neutral-50 p-4 rounded-xl border border-neutral-150 text-center">
                                    <div>
                                        <span className="text-[10px] text-neutral-400 font-bold block mb-1">
                                            {isEnglish ? "PRODUCT" : "NÔNG SẢN"}
                                        </span>
                                    ...
                                    <div className="md:border-l border-neutral-200">
                                        ...
                                        <span className="text-xs font-extrabold text-neutral-800">
                                            {contract.don_gia.toLocaleString(
                                                "vi-VN",
                                            )}{" "}
                                            đ
                                        </span>
                                ...
                                <div className="space-y-3.5 text-xs">
                                    <div className="flex justify-between border-b border-neutral-100 pb-2">
                                        ...
                                        <span className="font-mono text-neutral-800 text-[11px] font-semibold">
                                            {contract.dia_chi_vi_escrow ||
                                                "MOCK_ESCROW_PDA_VAULT"}
                                        </span>
                                    </div>
                                    <div className="flex justify-between border-b border-neutral-100 pb-2">
                                        <span className="text-neutral-450">
                                    ...
                                    <div className="flex justify-between">
                                        <span className="text-neutral-450">
                                            {isEnglish
                                                ...
                                                : "Hạn giao nhận cam kết:"}
                                        </span>
                                        <span className="font-bold text-neutral-800">
                                            {new Date(
                                                contract.han_giao_hang,
                                            ).toLocaleString("vi-VN")}
                                        </span>
                                    </div>

                                    <div className="border-t border-neutral-100 pt-3.5 mt-3.5 space-y-2.5">
                                        <p className="font-bold text-neutral-800 text-[11px] uppercase tracking-wider">
                                            Chữ ký số các bên
                                        </p>
                                            ...
                                            <div className="bg-neutral-50 p-2.5 rounded-lg border border-neutral-150 text-[11px] space-y-1">
                                                ...
                                                <span className="font-mono text-[9px] text-neutral-500 break-all block">
                                                    {contract.noi_dung_nhap_ai
                                                        ...
                                                        : contract.vi_nguoi_ban
                                                          ? `${contract.vi_nguoi_ban.slice(0, 6)}...${contract.vi_nguoi_ban.slice(-4)}`
                                                          : "Chưa có ví"}
                                                </span>
                                            </div>
                                            <div className="bg-neutral-50 p-2.5 rounded-lg border border-neutral-150 text-[11px] space-y-1">
                                                ...
                                                <span className="font-mono text-[9px] text-neutral-500 break-all block">
                                                    {contract.noi_dung_nhap_ai
                                                        ...
                                                        : contract.vi_nguoi_mua
                                                          ? `${contract.vi_nguoi_mua.slice(0, 6)}...${contract.vi_nguoi_mua.slice(-4)}`
                                                          : "Chưa có ví"}
                                                </span>
                                            </div>
                                        </div>
                            ...
                            {txSignature && (
                                <div className="p-3 bg-indigo-50/50 border border-indigo-150 rounded-xl text-xs space-y-1.5">
                                    <p className="font-bold text-indigo-700 flex items-center gap-1.5">
                                        <ShieldCheck size={14} /> Chữ ký Giao
                                        dịch Solana (Devnet):
                                    </p>
                                    <a
                        ...
                        isNongDan && (
                            <div className="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm space-y-3">
                                <h4 className="text-xs font-bold text-neutral-800 uppercase tracking-wider">
                                    Kịch bản C: Rút tiền quá hạn (Timeout)
                                </h4>
                                <p className="text-xs text-neutral-500">
                                    Người mua im lặng và quá hạn cam kết giao
                                    nhận. Bạn có quyền đòi lại tiền cọc.
                                </p>
                                <TimeoutClaimButton
                        ...
                        <div className="bg-white border border-neutral-200 rounded-2xl shadow-sm flex flex-col overflow-hidden">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="group relative flex items-center">
                                        ...
                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 hidden group-hover:block bg-slate-900/95 backdrop-blur-sm text-white text-[11px] rounded-lg p-2.5 shadow-xl z-50 text-center border border-white/10 transition-all leading-normal font-semibold">
                                            Lịch sử đàm phán được AI xác thực và
                                            lưu trên blockchain
                            ...
                            <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-50/50 max-h-[400px]">
                                {transcripts.map((msg, idx) => {
                                    const isSenderNongDan =
                                        msg.vi_nguoi_noi ===
                                        contract.vi_nguoi_ban;
                                    const senderName = isSenderNongDan
                                        ? contract.seller_name
                                        : contract.buyer_name;
                                    const senderRole = isSenderNongDan
                                        ? "Nông dân"
                                        : "Thương lái";
                                    const displayName =
                                    ...
                                    return (
                                        <div
                                            key={idx}
                                            className={`flex flex-col ${isSenderNongDan ? "items-start" : "items-end"}`}
                                        >
                                            <span className="text-[10px] font-bold text-neutral-450 mb-1 px-1 uppercase tracking-wider">
                                                {displayName}
                                            </span>
                                            <div
                                                className={`px-4 py-2.5 rounded-2xl max-w-[85%] text-[13px] leading-relaxed shadow-sm ${
                                                    isSenderNongDan
                        ...
                        <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm space-y-5">
                            <div className="flex items-center gap-2 border-b border-neutral-100 pb-2">
                                ...
                                <h3 className="text-xs font-bold text-neutral-800 uppercase tracking-wider">
                                    Quy trình Nghiệm thu & Giao nhận
                                </h3>
                            </div>
                            ...
                            {!isNongDan ? (
                                <div className="space-y-5">
                                            ...
                                            !isBypassDeadline ? (
                                                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex flex-col items-center justify-center text-center animate-fadeIn">
                                                    ...
                                                    <h4 className="text-sm font-bold text-amber-800 mb-1">
                                                        Chưa tới hạn giao hàng
                                                    </h4>
                                                    <p className="text-xs text-amber-700 max-w-md">
                                                        Hợp đồng này có hạn giao
                                                        hàng là{" "}
                                                        <strong>
                                                            {new Date(
                                                                contract.han_giao_hang,
                                                        ...
                                                        . Vui lòng chờ đến ngày
                                                        giao nhận thực tế để
                                                    ...
                                                    <button
                                                        ...
                                                        className="mt-3 text-[10px] text-amber-600/60 hover:text-amber-600 underline transition-colors cursor-pointer border-none bg-transparent"
                                                    >
                                                        [Demo Bypass] Bỏ qua
                                                        kiểm tra thời gian để
                                                        test ngay
                                                    </button>
                                                </div>
                                            ) : (
                                                <>
                                                    <p className="text-xs text-neutral-500 leading-relaxed">
                                                        Lô hàng nông sản đã được
                                                        chuyển tới điểm giao
                                                        nhận. Bạn đã kiểm nghiệm
                                                        thực tế chưa? Vui lòng
                                                        chọn tình trạng nghiệm
                                                        thu bên dưới:
                                                    </p>
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                                                        ...
                                                        >
                                                            <CheckCircle2
                                                                size={24}
                                                                className="text-neutral-400 group-hover:text-emerald-600 transition-colors mb-2"
                                                            />
                                                            <span className="font-bold text-xs text-neutral-850 block">
                                                                Nghiệm thu đạt
                                                                chuẩn
                                                            </span>
                                                        ...
                                                        >
                                                            <AlertCircle
                                                                size={24}
                                                                className="text-neutral-400 group-hover:text-red-600 transition-colors mb-2"
                                                            />
                                                            <span className="font-bold text-xs text-neutral-850 block">
                                                                Phát hiện lỗi
                                                                nông sản
                                    ...
                                    {inspectionDecision === "ok" && (
                                        <div className="space-y-4 text-center py-4 bg-emerald-50/30 rounded-xl border border-emerald-100">
                                            <CheckCircle2
                                                size={32}
                                                className="text-[#15803D] mx-auto animate-bounce"
                                            />
                                            <div>
                                                <h4 className="font-bold text-neutral-800 text-xs uppercase">
                                                    Xác nhận Giải ngân 100%
                                                </h4>
                                                <p className="text-[11px] text-neutral-400 max-w-xs mx-auto mt-1">
                                                    Hệ thống sẽ chuyển toàn bộ
                                                    SOL đang khóa trong két sắt
                                                    Solana sang ví của Nông dân.
                                                </p>
                                            </div>
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                >
                                                    <span className="text-[11px] text-[#d32f2f] font-extrabold flex items-center gap-1.5">
                                                        ...
                                                        Đang kích hoạt luồng
                                                        Khiếu nại (Kịch bản B)
                                                    </span>
                                                    <button
                                                        onClick={() =>
                                                            setInspectionDecision(
                                ...
                                <div className="p-6 text-center border border-dashed border-neutral-200 rounded-xl text-neutral-400 space-y-2">
                                    <Clock
                                        size={28}
                                        className="mx-auto text-neutral-350 animate-spin"
                                    />
                                    <p className="text-xs font-bold">
                                        Đang đợi Thương lái nghiệm thu...
                                    </p>
                                    <p className="text-[10px] text-neutral-450">
                                        Thương lái sẽ kiểm định chất lượng, xác
                                        nhận giải ngân 100% hoặc gửi báo cáo
                                        chất lượng nếu có sai lệch.
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm space-y-4">
                                <div className="flex items-center justify-between border-b border-neutral-100 pb-2">
                                    <span className="text-xs font-bold text-red-650 uppercase tracking-wider flex items-center gap-1.5">
                                        ...
                                        {isEnglish
                                            ...
                                            : "Chi tiết lỗi nghiệm thu"}
                                    </span>
                                    <div className="flex items-center gap-2">
                                        {!dispute.nguoi_ban_dong_y &&
                                            !dispute.nguoi_mua_dong_y &&
                                            dispute.trang_thai !==
                                                "da_giai_ngan" && (
                                                <button
                                                    onClick={async () => {
                                ...
                                <div className="grid grid-cols-2 gap-4 text-xs">
                                    <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-150">
                                        <span className="text-[10px] text-neutral-400 block font-bold mb-0.5">
                                            {isEnglish
                                                ? "RECEIVED QUANTITY"
                                                : "SỐ LƯỢNG THỰC NHẬN"}
                                        </span>
                                        <span className="font-extrabold text-neutral-800">
                                            {dispute.so_luong_thuc_nhan}{" "}
                                            {contract.don_vi_tinh}
                                        </span>
                                    </div>
                                    <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-150">
                                            ...
                                            <div className="flex gap-2">
                                                {dispute.danh_sach_url_anh.map(
                                                    (
                                                        ...
                                                        <div
                                                            key={index}
                                                            className="w-16 h-16 rounded-lg border border-neutral-250 overflow-hidden"
                                                        >
                                                            <img
                                                                src={url}
                                                                alt="Evidence defect"
                                ...
                                <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-sm animate-pulse">
                                    <div className="text-orange-800 text-xs">
                                        <p className="font-bold flex items-center gap-1.5">
                                            <Clock size={14} /> Chờ xác nhận Báo
                                            cáo lỗi
                                        </p>
                                        <p className="text-[10px] mt-0.5 opacity-90">
                                            Nông dân cần xác nhận Báo cáo nghiệm
                                            thu thực tế này thì Trọng tài AI mới
                                            được quyền phân xử.
                                        </p>
                                    </div>
                                    {isNongDan ? (
                            ...
                            {dispute.nguoi_ban_da_duyet ? (
                                <div className="space-y-6 animate-fadeIn">
                                    ...
                                    {dispute.ty_le_giai_ngan_ai_de_xuat !==
                                    null ? (
                                        <SettlementProposal
                                            proposedRatio={
                                                dispute.ty_le_giai_ngan_ai_de_xuat
                                            ...
                                            payoutAmount={
                                                dispute.so_tien_giai_ngan_de_xuat
                                            }
                                            refundAmount={
                                                dispute.so_tien_hoan_lai_de_xuat
                                            }
                                            note={
                                                dispute.ghi_chu_chat_luong ||
                                                (isEnglish
                                                    ? "AI calculated this from moisture, broken grains, and actual shortfall."
                                                    : "AI đã tính toán dựa trên độ ẩm, hạt lép và sản lượng hao hụt thực tế.")
                                            }
                                            isEnglish={isEnglish}
                                        />
                                    ) : null}
                                            ...
                                            <div className="text-center pb-2 border-b border-neutral-100">
                                                <h4 className="text-xs font-bold text-neutral-800 uppercase tracking-wider">
                                                    {isEnglish
                                                        ...
                                                        : "Đồng thuận quyết định giải quyết"}
                                                </h4>
                                                <p className="text-[10px] text-neutral-400 mt-1">
                                                    {isEnglish
                                                        ? "Both sides must confirm to execute the automatic wallet split."
                                                        : "Cả nông dân và thương lái cần xác nhận để thực thi lệnh chia tiền ví tự động."}
                                                </p>
                                            </div>
                    ...
                    {contract.trang_thai !== "da_khoa_tien" &&
                        contract.trang_thai !== "dang_tranh_chap" && (
                            <div className="bg-white border border-neutral-200 rounded-2xl p-8 text-center space-y-4 shadow-sm">
                                ...
                                <div>
                                    <h4 className="font-extrabold text-neutral-800 text-[14px] uppercase tracking-wide">
                                        {isEnglish
                                            ? "Escrow account settled"
                                            : "Tài khoản Ký quỹ đã thanh lý"}
                                    </h4>
                                    <p className="text-xs text-neutral-450 mt-1.5">
                                        {isEnglish
                                            ? "All payment obligations for this contract have been completed safely."
                                        ...
                                        <span className="font-bold text-[#15803D]">
                                            ...
                                            "da_giai_quyet"
                                                ...
                                                : contract.trang_thai ===
                                                    "da_xac_nhan"
                                                  ? isEnglish
                                                      ? "Contract completed (100% paid out)"
                                                      : "Hợp đồng đã hoàn tất (Giải ngân 100%)"
                                                  : contract.trang_thai}
                                        </span>
                                    </p>
                                    {(contract.trang_thai === "da_giai_quyet" ||
                                        contract.trang_thai ===
                                            "da_xac_nhan") && (
