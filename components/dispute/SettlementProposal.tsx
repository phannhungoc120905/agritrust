      ...
      <div className="space-y-1.5 py-1">
        <div className="w-full bg-slate-100 rounded-full h-3.5 flex overflow-hidden shadow-inner border border-slate-200">
          <div 
            ...
            title={`Giải ngân người bán: ${sellerPercent.toFixed(0)}%`}
          ></div>
          <div 
            style={{ width: `${buyerPercent}%` }} 
            className="bg-slate-400 h-full transition-all duration-500"
            title={`Hoàn trả người mua: ${buyerPercent.toFixed(0)}%`}
          ></div>
        </div>
        <div className="flex justify-between items-center text-[10.5px] text-slate-500 font-bold px-1">
          <span className="text-emerald-700">Người bán: {sellerPercent.toFixed(0)}%</span>
          <span className="text-slate-600">Người mua: {buyerPercent.toFixed(0)}%</span>
        </div>
      </div>
